...
### מדריך כתיבה (Wizard)
- **Wizard צעד-צעד** (`src/guide-wizard.js`) — 8 שלבים: ברוכים הבאים → פרטים אישיים → תקציר → ניסיון → השכלה → מיומנויות → שפות → סיכום
- כל שלב כולל טיפים ממוקדים, דוגמה טובה ודוגמה חלשה
- מדד השלמה אוטומטי עם progress bar
- שלב סיכום עם checklist אינטראקטיבי

### טיפים מתקדמים לפי שדה
- **Field-level tips** — hint, good example, bad example לכל שדה בעורך
- פעלי פעולה מקובצים לפי קטגוריות (ניהול, פיתוח, שיפור, תקשורת, מחקר, מכירות)
- נוסחאות להישגים: 4 תבניות כתיבה עם דוגמאות
- זיהוי מילים חלשות: 10 ביטויים שכיחים עם חלופות חזקות
- ולידציות: אימייל, טלפון, אורך תקציר, ניסיון ללא תיאור

### תצוגת A4 מפוצלת
- **תצוגה מקדימה מפוצלת** — עמודי A4 נפרדים במקום דף אחד ארוך
- כל עמוד מציג חלון צפייה (viewport) בגובה 297mm עם `overflow: hidden`
- מספרי עמודים מתחת לכל עמוד ("עמוד 1 מתוך 2")
- גובה A4 נמדד דינמית ומתעדכן ב-resize
- תואם לייצוא PDF/PNG/JPEG ללא שינויים

### רספונסיביות מלאה (מובייל)
- **הדר רספונסיבי** — לוגו קטן, כפתורי PNG/JPEG/תצוגה מלאה מוסתרים במובייל
- **טאבים מובייל** — "עריכת פרטים" / "תצוגה מקדימה" עם מעבר חלק
- **סקיילינג רספונסיבי** — CSS custom property `--s` עם media queries (0.42–0.96)
- **טולבר בשורה אחת** — גלילה אופקית (`overflow-x-auto`) במקום שבירת שורות
- הסרת באנר "לא מותאם למובייל"
...
- טיפים לכתיבה כבועות דיבור + טיפים מתקדמים לפי שדה
- מדריך כתיבה (Wizard) צעד-צעד
- תצוגת A4 מפוצלת לעמודים נפרדים
- ממשק רספונסיבי מלא (מובייל + דסקטופ)
- דוגמאות מקצועיות (4 מקצועות)
- GA4 עם consent mode v2
- באנר הסכמה תקני (GDPR)
- ווידג'ט נגישות (תקן 5568)
- דף פרטיות + הצהרת נגישות
...
src/
  ...
  writing-tips.js   # טיפים לכתיבה + ולידציות + פעלי פעולה
  guide-wizard.js   # מדריך כתיבה צעד-צעד (Wizard)
