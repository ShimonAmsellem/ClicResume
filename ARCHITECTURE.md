...
- טיפים לכתיבה לפי סקציה (section-level tips).
- טיפים מתקדמים לפי שדה (field-level tips): hint, good example, bad example לכל שדה.
- פעלי פעולה מקובצים לפי קטגוריות (ניהול, פיתוח, שיפור, תקשורת, מחקר, מכירות).
- נוסחאות להישגים: 4 תבניות כתיבה עם דוגמאות.
- זיהוי מילים חלשות (weak words): 10 ביטויים שכיחים עם חלופות חזקות.
- ולידציות: אימייל, טלפון, אורך תקציר, ניסיון ללא תיאור, זיהוי מילים חלשות בתקציר ובניסיון.
...
### 10. מדריך כתיבה (`src/guide-wizard.js`)
- Wizard צעד-צעד למשתמשים שכותבים קורות חיים בפעם הראשונה.
- 8 שלבים: ברוכים הבאים → פרטים אישיים → תקציר → ניסיון → השכלה → מיומנויות → שפות → סיכום.
- כל שלב כולל: טיפים ממוקדים, דוגמה טובה, דוגמה חלשה (בסקציות רלוונטיות), ושדות קלט.
- מדד השלמה אוטומטי: בודק כמה סקציות מולאו ומציג אחוז.
- שלב הסיכום מציג checklist אינטראקטיבי עם אפשרות לחזור לכל סקציה.
- נפתח ידנית מכפתור "מדריך כתיבה" בסרגל הכלים.

### 11. תצוגת A4 מפוצלת
- התצוגה המקדימה מפוצלת לעמודי A4 נפרדים (page frames).
- כל עמוד מציג חלון צפייה (viewport) בגובה 297mm עם `overflow: hidden`.
- התוכן מרונדר במלואו בכל frame ומוזז באמצעות `transform: translateY(...)`.
- מספרי עמודים מוצגים מתחת לכל עמוד כשיש יותר מעמוד אחד.
- גובה A4 בפיקסלים נמדד דינמית ומתעדכן ב-resize.
- המבנה תומך בייצוא PDF/תמונה: קוד הייצוא משכפל את ה-`.cv-page` הראשון ומאפס transforms.
- סקיילינג רספונסיבי עם CSS custom property `--s` ומדיה-queries לגדלי מסך שונים.

### 12. אפליקציה ראשית (`src/app.js`)
...
│   ├── guide-wizard.js     # Step-by-step writing wizard
│   ├── templates/
